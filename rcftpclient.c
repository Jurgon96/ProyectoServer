...
int main(int argc,char *argv[]) {
	...
	/* leer parametros de entrada */
    	initargs(argc,argv,&verb,&alg,&timeout,&dest,&port);
	...
	switch(alg) {
		...
		case 2: alg_ventana_anunciada(sock,servinfo); break;
		case 3: alg_congestion(sock,servinfo); break;
...
void printuso(char *progname) {
   fprintf(stderr,"Uso: %s [-v] -a[alg] -T[timeout]] -d<dirección> -p<puerto>\n",progname);
	...
	fprintf(stderr,"      2\t\tAlgoritmo de ventana anunciada \n");
	fprintf(stderr,"      3\t\tAlgoritmo de control de congestión\n");
...
/**************************************************************************/
void initargs(int argc, char **argv, char *verb, int* alg, unsigned long* timeout, char** dest, char** port) {
    char *progname = *argv;
    ...
    *port=NULL;
	...
	if (*verb) {
		fprintf(stderr,"Valores de parámetros: a=%d, T=%ld, d=%s, p=%s\n",*alg,*timeout,*dest,*port);
	}	
}
